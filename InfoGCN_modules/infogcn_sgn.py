...
class InfoGCN_jvb(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3,
        ...
        super(InfoGCN_jvb, self).__init__()
        ...
        self.data_bn = nn.BatchNorm1d(base_channel * num_point)    #nn.BatchNorm1d(num_person * base_channel * num_point)
    ...
    def forward(self, x):
        ...
        pos, vel, bone = self.multi_input_SGN(x, self.connect_joint)                #[4, 6, 64, 25, 2], [4, 6, 64, 25, 2], [4, 6, 64, 25, 2]
        pos = pos.permute(0, 1, 3, 2)
        vel = vel.permute(0, 1, 3, 2)
        bone = bone.permute(0, 1, 3, 2)

        N, C, T, V = pos.size() #N, C, T, V, M = pos.size()  #x.size() # [4, 3, 64, 25, 2]
        pos = rearrange(pos, 'n c t v -> (n t) v c', t=T, v=V).contiguous()
        # x = self.A_vector.to(x.device).expand(N*M*T, -1, -1) @ x                  #buggeed
        pos = self.A_vector.type(torch.cuda.FloatTensor).expand(N * T, -1, -1) @ pos

        pos = self.to_joint_embedding(pos)
        pos += self.pos_embedding[:, :self.num_point]
        pos = rearrange(pos, '(n t) v c -> n (v c) t', v=V, t=T).contiguous()

        pos = self.data_bn(pos)
        pos = rearrange(pos, 'n (v c) t -> n c t v', t=T, v=V).contiguous()
        ##### added: EfficientGCN like network:
        pos = self.j1(pos)
        pos = self.j2(pos)
    ######### Velocity
        vel = rearrange(vel, 'n c t v -> (n t) v c', t=T, v=V).contiguous()
        vel = self.A_vector.type(torch.cuda.FloatTensor).expand(N * T, -1, -1) @ vel

        vel = self.to_velocity_embedding(vel)
        vel += self.pos_embedding[:, :self.num_point]
        vel = rearrange(vel, '(n t) v c -> n (v c) t', v=V, t=T).contiguous()

        vel = self.data_bn(vel)
        vel = rearrange(vel, 'n (v c) t -> n c t v', t=T, v=V).contiguous()
        vel = self.v1(vel)
        vel = self.v2(vel)
    ######### Bone
        bone = rearrange(bone, 'n c t v -> (n t) v c', t=T, v=V).contiguous()
        bone = self.A_vector.type(torch.cuda.FloatTensor).expand(N * T, -1, -1) @ bone

        bone = self.to_bone_embedding(bone)
        bone += self.pos_embedding[:, :self.num_point]
        bone = rearrange(bone, '(n t) v c -> n (v c) t', v=V, t=T).contiguous()

        bone = self.data_bn(bone)
        bone = rearrange(bone, 'n (v c) t -> n c t v', t=T, v=V).contiguous()
        bone = self.b1(bone)
        bone = self.b2(bone)
    ...
    def multi_input_SGN(self, data, conn):
        ### this function "multi_input_SGN_2" contains "velocity_mx_prime" compared with "multi_input_SGN"
        N, C, V, T = data.size()
        if torch.cuda.is_available():  # 'GPU'
        ...
        for i in range(C):
            bone[:, C + i, :, :] = torch.arccos(bone[:, i, :, :] / bone_length)
            # bone[:, C + i, :, :] = torch.acos(bone[:, i, :, :] / bone_length)  # acos(): torch=1.3.0 | arcoos(): torch>1.3.0
