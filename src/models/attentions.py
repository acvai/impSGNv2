...
class ST_Joint_Att(nn.Module):
    ...
    def forward(self, x):
        N, C, T, V = x.size()
        x_t = x.mean(3, keepdims=True)			#bs, c, 20, 1
        x_v = x.mean(2, keepdims=True).transpose(2, 3)	#bs, c, 1, 25 ->transpose-> (bs, c, 25, 1)
        x_att = self.fcn(torch.cat([x_t, x_v], dim=2)); print('**** 1-2): ', torch.cat([x_t, x_v], dim=2).size(), x_att.size())  ##*** 1-2): [4, 16, 45, 1] [4, 8, 45, 1])
        x_t, x_v = torch.split(x_att, [T, V], dim=2); print('****3) ', x_t.size(), x_v.size())
        x_t_att = self.conv_t(x_t).sigmoid()
        x_v_att = self.conv_v(x_v.transpose(2, 3)).sigmoid(); print('****4) ', x_t_att.size(), x_v_att.size())
        x_att = x_t_att * x_v_att; print('****3) ', x_att.size())
        return x_att
...
if __name__ == '__main__': 
	...
	att = ST_Joint_Att(16, 2, False)
	print(att)
	
	x = torch.randn(4, 16, 20, 25)
