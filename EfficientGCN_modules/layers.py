...
if __name__ == '__main__':
    ...
    print(graph.A.shape, type(graph.A), type(torch.tensor(graph.A)))


    #### Works
    # SG = SpatialGraphConv(128, 256, 1, True, False, torch.Tensor(graph.A))
    # print(SG)
    # bs, c, v, t = 2, 128, 25, 20
    # x = torch.randn(bs, c, t, v)
    # # x = x.cuda()
    # # SG = SG.cuda()
    #
    # y = SG(x)
    # print('**** 3) x, y: ', x.size(), y.size())
    # assert False, 'bbbbbbbbbb'
    #### Works
    ...
    #####
    att = ST_Joint_Att(128, reduct_ratio=2, **kwargs)
    y = att(x)
    print('******** x, y: ', x.size(), y.size())
    assert False, 'cccccccccccc'

    SGL = Spatial_Graph_Layer(128, 258, 1, **kwargs)
    print(SGL)
